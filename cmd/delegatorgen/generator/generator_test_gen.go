...
func (g *Generator) generateMockCache(gf *genkit.GeneratedFile, iface *genkit.Interface) {
	...
	gf.P("getCalls ", genkit.GoImportPath("sync/atomic").Ident("Int64"))
	gf.P("setCalls ", genkit.GoImportPath("sync/atomic").Ident("Int64"))
	gf.P("delCalls ", genkit.GoImportPath("sync/atomic").Ident("Int64"))
	gf.P("}")
...
func (g *Generator) generateCacheMethodTests(gf *genkit.GeneratedFile, iface *genkit.Interface, m *genkit.Method, mockName, mockCacheName, delegatorType string) {
	...
	gf.P("_, _ = svc.", m.Name, "(", formatTestArgs(m.Params), ")")
	gf.P("initialSetCalls := cache.setCalls.Load()")
	gf.P()
	...
	gf.P("if cache.setCalls.Load() <= initialSetCalls {")
	gf.P("t.Errorf(\"expected async refresh to update cache, setCalls: initial=%d, after=%d\", initialSetCalls, cache.setCalls.Load())")
	gf.P("}")
	...
	if hasErrorReturn(m.Results) {
		...
		gf.P("if cache.setCalls.Load() != initialSetCalls {")
		gf.P("t.Errorf(\"expected cache not to be updated on refresh error, setCalls: initial=%d, after=%d\", initialSetCalls, cache.setCalls.Load())")
		gf.P("}")
		gf.P("}")
	}
}
...
func (g *Generator) generateCacheEvictMethodTest(gf *genkit.GeneratedFile, iface *genkit.Interface, m *genkit.Method, mockName, mockCacheName, delegatorType string) {
	...
	if hasErrorReturn(m.Results) {
		...
		gf.P("if cache.delCalls.Load() != 0 {")
		gf.P("t.Errorf(\"expected cache.Delete not to be called on error, got %d calls\", cache.delCalls.Load())")
		gf.P("}")
		gf.P("}")
	}
}
