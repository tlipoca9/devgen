...
### 12. @oneof_enum(EnumType) - Enum Type Validation

Validates that field value is a valid value of the specified enum type. Unlike `@oneof`, `@oneof_enum` automatically uses the `EnumTypeEnums.Contains()` method generated by enumgen, avoiding the need to update `@oneof` annotations when adding new enum values.
...
**Cross-package enum (full import path, auto-adds import):**
```go
// validategen:@validate
type Request struct {
    // validategen:@oneof_enum(github.com/myorg/pkg/types.Status)
    Status types.Status  // Auto imports "github.com/myorg/pkg/types"
}

// Generated code automatically adds import:
...
type Request struct {
    ...
    Status mytypes.Status  // Uses specified alias
}
...
**Advantage**: When adding new enum values (e.g., `RoleModerator`), you only need to modify the enum definition, not the `@oneof_enum` annotation, following the single modification principle.

---

### 13. @email - Email Format
...
### 20. @duration_min(duration) - Minimum Duration
...
### 21. @duration_max(duration) - Maximum Duration
...
### 22. @duration + @duration_min + @duration_max Combined
...
### 31. @format(type) - Format Validation
...
| `@oneof_enum(EnumType)` | enum type name | enum type | Enum type validation |
