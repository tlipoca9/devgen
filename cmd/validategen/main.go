...
func rootCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:     "validategen [packages]",
		Short:   "Generate Validate() methods for structs",
		Long:    `validategen generates Validate() methods for Go structs annotated with validategen:@validate.`,
		Version: fmt.Sprintf("%s (%s) %s", ver, commit, date),
		Example: `  validategen ./...              # all packages
  validategen ./pkg/model        # specific package`,
	...
	cmd.SetVersionTemplate(fmt.Sprintf("validategen %s (%s) %s\n", ver, commit, date))
...
func run(_ *cobra.Command, args []string) error {
	...
	if len(files) == 0 {
		log.Warn("No types with validation found")
		return nil
	}
